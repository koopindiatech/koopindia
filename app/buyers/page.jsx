...
import BuyerCard from "@/components/ui/cards/BuyerCard";
...
export default function BuyersPage() {
  ...
  useEffect(() => {
    const fetchBuyers = async () => {
      setLoading(true);
      try {
        // Fetch up to 100 buyers for local filtering
        const q = query(collection(db, "buyers"), orderBy("createdAt", "desc"), limit(100));
  ...
  const filteredBuyers = buyers.filter(b => {
    ...
    const req = (b.requirement || "").toLowerCase();
    const ind = (b.industry || "").toLowerCase();
    const state = (b.state || "").toLowerCase();
    const city = (b.city || "").toLowerCase();
    return name.includes(q) || req.includes(q) || ind.includes(q) || state.includes(q) || city.includes(q);
  });
        ...
        <div className="bg-white border-b border-gray-100 py-10 px-4 sm:px-6">
          <div className="max-w-7xl mx-auto">
            ...
            <div className="relative max-w-2xl">
              ...
              <input
                type="text"
                placeholder="Search by company name, requirements, or location..."
                value={searchQuery}
        ...
        <div className="max-w-7xl mx-auto py-10 px-4 sm:px-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
            {loading ? (
              ...
              filteredBuyers.map(b => <BuyerCard key={b.id} b={b} />)
            ) : (
              <div className="col-span-full flex flex-col items-center justify-center py-20 text-gray-400">
                <div className="text-5xl mb-4">🔍</div>
                <h3 className="text-xl font-bold text-gray-700 mb-2">No buyers found</h3>
                <p className="text-sm">We couldn't find any buyers matching "{searchQuery}"</p>
                <button 
