...
/* Removed homeGridCls since we are using flex center for all grids */
...
export default function SellerPublicPage() {
    ...
    (async () => {
      try {
        ...
        if (!snap.empty) {
          const d = snap.docs[0];
          const data = d.data();
          const proxyUrl = (u) => (u && typeof u === "string" && u.startsWith("http") && u.includes("firebase") ? `/api/img?url=${encodeURIComponent(u)}` : u);
          const cleanData = {
            id: d.id,
            ...data,
            ...
            heroBanners: (data.heroBanners || []).map((b) => (typeof b === "object" ? { ...b, url: proxyUrl(b.url || b.imageUrl) } : proxyUrl(b))),
          ...
          setSeller(cleanData);
          await updateDoc(doc(db, "sellers", d.id), { views: increment(1) });
        }
  ...
  const hBadges = seller.heroBadges || [];
  ...
  /* ── Centered grid for products — 1 item: centered; multiple: wrap ── */
  const ProdGrid = ({ list }) => (
    <div className={`flex flex-wrap gap-6 ${list.length === 1 ? "justify-center" : "justify-center"}`}>
      {list.map((p, i) => (
  ...
  /* ── Nav inner content — reused in desktop + mobile ── */
  const NavItems = () => {
                ...
                <div className="absolute top-full left-1/2 -translate-x-1/2 mt-1 bg-white rounded-2xl shadow-2xl border border-gray-100 z-50 overflow-hidden" style={{ width: "260px" }}
                  ...
                  <div className="p-2 max-h-[360px] overflow-y-auto">
                    {products.slice(0, 8).map((p, i) => (
                      <button key={i} onClick={() => { setSelProd(p); setProdDropOpen(false); }}
    ...
    <div className="min-h-screen bg-white" style={{ fontFamily: "'Inter','Segoe UI',sans-serif" }}>
      ...
      {tab === "home" && !selProd && (
        <div>
              ...
              <div className="max-w-7xl mx-auto">
                <div className="text-center mb-10">
                  <h2 className="text-2xl sm:text-3xl font-black text-gray-900">
                    Featured <span style={{ color: pc }}>Products</span>
                  </h2>
                </div>
          ...
          {/* Certifications — same-size cards */}
      ...
      {tab === "about" && !selProd && (
        <div style={{ backgroundColor: aboutBg }}>
          ...
          {/* About text — full-width bg, inner max-w-7xl */}
                ...
                <div className="lg:col-span-3 bg-white p-10">
                  ...
                  <form onSubmit={onSubmit} className="space-y-5">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <div>
                        ...
                        <input type="text" required placeholder="Enter your name" value={contactForm.name} onChange={e => setContactForm(p => ({ ...p, name: e.target.value }))}
                          className="w-full px-4 py-3.5 rounded-xl border-2 border-gray-100 text-sm outline-none focus:border-current transition bg-gray-50 placeholder:text-gray-400 font-medium"
                          style={{ '--focus-color': pc }} onFocus={e => e.target.style.borderColor = pc} onBlur={e => e.target.style.borderColor = '#f3f4f6'} />
                      </div>
                      <div>
                    ...
                    <div>
                      ...
                      <input type="email" required placeholder="Enter your email address" value={contactForm.email} onChange={e => setContactForm(p => ({ ...p, email: e.target.value }))}
                        className="w-full px-4 py-3.5 rounded-xl border-2 border-gray-100 text-sm outline-none transition bg-gray-50 placeholder:text-gray-400 font-medium"
                        onFocus={e => e.target.style.borderColor = pc} onBlur={e => e.target.style.borderColor = '#f3f4f6'} />
                    </div>
                    ...
                    <button type="submit"
                      className="w-full text-white font-black py-4 rounded-xl shadow-lg hover:opacity-90 hover:-translate-y-0.5 transition-all text-sm cursor-pointer tracking-wide"
                      style={{ backgroundColor: pc }}>
                      Send Message →
                    </button>
                  </form>
