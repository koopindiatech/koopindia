...
export default function KoopIndiaHomepage() {
  ...
  return (
    <div className="bg-white text-gray-800 font-sans" style={{ fontFamily: "'Inter','Segoe UI',sans-serif" }}>
      ...
      <section className="bg-white py-14 px-4 sm:px-6 relative border-t border-gray-100">
        <div className="max-w-7xl mx-auto">
          <h2 className="text-2xl font-black text-[#1e3a5f] text-center mb-8 tracking-tight">EXPLORE ALL CATEGORIES</h2>
          <div className="grid grid-cols-3 sm:grid-cols-5 md:grid-cols-6 lg:grid-cols-8 xl:grid-cols-12 gap-3">
            {CATEGORIES.map(c => (
              <button key={c.label} onClick={() => { 
                  setCatSel(c.label); 
                  setSubCatSel("All");
                  performSearch(search, c.label, "All", stateSel);
                  document.getElementById('sellers-section')?.scrollIntoView({ behavior: 'smooth' });
                }}
                className={`flex flex-col items-center gap-2 cursor-pointer group p-2 rounded-xl transition-all ${catSel === c.label ? "bg-orange-100 ring-2 ring-orange-400" : "hover:bg-orange-50"}`}>
                <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-orange-50 to-orange-100 border border-orange-100 flex items-center justify-center text-2xl shadow-sm group-hover:shadow-md group-hover:scale-110 transition-all">
                  {c.emoji}
                </div>
